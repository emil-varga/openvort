...
/*
 * Euler-steps the tangle in 'tangle' with time step dt and saves the result to
 * 'result'.
 *
 * Optionally can use externally supplied velocities use_vel. Pass NULL to use
 * standard vortex velocity.
 */
void euler_step2(struct tangle_state *result, const struct tangle_state *tangle,
                 double dt, const struct vec3d *use_vel) {
  int k;
...
void rk4_step2(struct tangle_state *result, const struct tangle_state *tangle,
               double t, double dt) {
  int N;
  struct tangle_state rk_state[3]; // k2 through k4, we already have k1 in
                                   // tangle
...
int connect_to_wall(struct tangle_state *tangle, int k, int wall,
                    node_status_t pin_mode, double time);

/*
  Run through all the pairs of nodes and check their distance if they are not
  immediate neighbors. Reconnect them if they are close enough. Also reconnect
  points close to a solid wall.

  This could, and should, in the future include some smarter estimation of
  possibility of a reconnections -- i.e., where in the BSP tree the two nodes
  are and only check them if can, in principle, be close enough.
 */
int reconnect(struct tangle_state *tangle, double t, double rec_dist,
              double rec_angle) {
  int k, l;
  ...
  for (int wall = 0; wall < 6; ++wall) {
    ...
    for (k = 0; k < tangle->N; ++k) {
      if (tangle->status[k].status == PINNED ||
          ...
          tangle->status[k].status == EMPTY || tangle->recalculate[k] > 0)
        continue;
  ...
  /*
   * Wall reconnection can create small loops that could be broken by the domain
   * killing below which can cause the simulation to crash. These loops need to
   * be removed.
   */
  if (Nrecs > 0) {
  ...
  /*
   * Eliminate all points that are active and outside the walls. This can
   * potentially happen if the time step is too long and more than one
   * discretisation point gets outside the domain.
   */
  int domain_killed = 0;
  for (int wall = 0; wall < 6; ++wall) {
    if (tangle->box.wall[wall] == WALL_OPEN ||
  ...
  for (k = 0; k < tangle->N; ++k) {
    ...
    if (tangle->status[k].status == EMPTY ||
        ...
        tangle->status[k].status == PINNED_SLIP || tangle->recalculate[k])
    ...
    for (l = k + 1; l < tangle->N; ++l) {
      if (tangle->status[l].status == EMPTY ||
          ...
          tangle->connections[k].reverse == l || tangle->recalculate[l])
        continue; // skip empty nodes and neighbors of k
                  // and nodes that went through a reconnection
      struct segment seg =
          seg_pwrap(&(tangle->vnodes[k]), &(tangle->vnodes[l]), &tangle->box);
      v1 = &seg.r1;
      v2 = &seg.r2;
      ...
      // pinned on the wall which have colinear points for which calculation of
      // the tangents fails. These result in tangents of 0 length. If the
      // tangent (which should be 1) is small we just ignore this point because
      // this loop will be removed on the next sweep of eliminate_small_loops
      // The 0.5 threshold is arbitrary. It will always be roughly 1 or roughly
      // 0.
      ...
      // do not bother with the Barnes-Hut here, passing NULL for the tree turns
      // it off even if enabled globally
      ...
      if (!do_reconnection(tangle, k, l))
        continue; // do_reconnect additionally checks if the total length
                  // doesn't increase
...
#define PSEG(x, y)                                                             \
  seg_pwrap(tangle->vnodes + x, tangle->vnodes + y, &tangle->box)
...
#undef PSEG
  ...
  cf1 = segment_len(&seg_kl) + segment_len(&seg_kflr) - segment_len(&seg_kkf) -
        segment_len(&seg_llr);

  cf2 = segment_len(&seg_kl) + segment_len(&seg_krlf) - segment_len(&seg_kkr) -
        segment_len(&seg_llf);
...
int check_wall(struct tangle_state *tangle, int k, int wall, double rdist) {
  ...
  case Y_L:
  case Z_L:
    return tangle->vnodes[k].p[idx[wall]] <
           (rdist + tangle->box.bottom_left_back.p[idx[wall]]);

  case X_H:
  case Y_H:
  case Z_H:
    return tangle->vnodes[k].p[idx[wall]] >
           (tangle->box.top_right_front.p[idx[wall]] - rdist);
...
int connect_to_wall(struct tangle_state *tangle, int k, int wall,
  ...
  update_velocity(tangle, k, time,
                  NULL); // do not bother with B-H for a small number of points
