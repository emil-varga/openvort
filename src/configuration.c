...
struct option options[] = {{"conf", required_argument, NULL, 'c'},
...
int load_conf_vector(config_t *cfg, const char *name, struct vec3d *v) {
  ...
  if (vector && config_setting_type(vector) == CONFIG_TYPE_ARRAY) {
    *v = vec3(config_setting_get_float_elem(vector, 0),
...
int setup_external_velocity(config_setting_t *v_conf_setting,
                            struct v_conf_t *v_conf) {
  double dval;
  const char *str;
  struct vec3d vval;
  ...
  for (int k = 0; k < (v_conf)->n_params; ++k) {
    ...
    case scalar_param:
      if (config_setting_lookup_float(v_conf_setting,
                                      (v_conf)->v_params[k].name, &dval))
    ...
    case vector_param:
      vec_cfg =
          config_setting_lookup(v_conf_setting, (v_conf)->v_params[k].name);
      for (int j = 0; j < 3; ++j)
  ...
  if (config_lookup_string(&cfg, "init_mode", &str)) {
    ...
    // single loop
      ...
      if (!config_lookup_int(&cfg, "loop_N", &N)) {
        fprintf(stderr,
                "Please specify number of discretisation points with loop_N.");
        goto failure;
      }
      ...
      if (!config_lookup_int(&cfg, "init_direction", &direction))
        goto failure;
      if (!config_lookup_float(&cfg, "init_lattice_constant",
                               &lattice_constant))
        goto failure;
      triangular_lattice(tangle, shells, direction, points_per_line,
                         lattice_constant);
...
void print_config(const struct tangle_state *tangle) {
  ...
  printf("time step                  = %g s\n"
         ...
         "loop remove outer          = %s\n"
         "domain = (%g, %g, %g), (%g, %g, %g)\n",
         global_dt, global_dl_min, global_dl_max,
         global_reconnection_angle_cutoff, global_small_loop_cutoff,
         global_frame_shot, global_num_threads,
         global_eliminate_origin_loops ? "On" : "Off",
         global_eliminate_loops_origin_cutoff,
         global_eliminate_outer_loops ? "On" : "Off",
         tangle->box.bottom_left_back.p[0], tangle->box.bottom_left_back.p[1],
         tangle->box.bottom_left_back.p[2], tangle->box.top_right_front.p[0],
         tangle->box.top_right_front.p[1], tangle->box.top_right_front.p[2]);
  ...
  if (global_use_BH) {
    ...
    printf("\tresolution        = %g\n"
           "\tquadtree          = %s\n"
           "\tgrain             = %g\n",
           global_BH_resolution, global_BH_quadtree ? "True" : "False",
           global_BH_grain);
  } else {
