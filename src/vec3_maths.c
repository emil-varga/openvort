...
const struct mat3d mat_identity = {.m = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};
...
int in_box(const struct domain_box *box, const struct vec3d *vec) {
  ...
  int in_box;
  in_box = vx > box->bottom_left_back.p[0] && vx < box->top_right_front.p[0] &&
...
struct segment seg_pwrap(const struct vec3d *r1, const struct vec3d *r2,
  ...
  struct segment seg = {.r1 = *r1, .r2 = *r2};
...
struct vec3d mirror_shift(const struct vec3d *v, const struct domain_box *box,
  ...
  switch (wall) {
  case X_L:
    wall_pos = box->bottom_left_back.p[0];
    break;
  case X_H:
    wall_pos = box->top_right_front.p[0];
    break;
  case Y_L:
    wall_pos = box->bottom_left_back.p[1];
    break;
  case Y_H:
    wall_pos = box->top_right_front.p[1];
    break;
  case Z_L:
    wall_pos = box->bottom_left_back.p[2];
    break;
  case Z_H:
    wall_pos = box->top_right_front.p[2];
    break;
  default:
    error("Bad wall %d", wall);
    wall_pos = -1;
...
/// @brief res = u x v
void vec3_cross(struct vec3d *res, const struct vec3d *u,
                const struct vec3d *v) {
  struct vec3d vv = *v;
  struct vec3d uu = *u;
...
/// @brief res = u*m
void vec3_mul(struct vec3d *res, const struct vec3d *u, double m) {
  for (int k = 0; k < 3; ++k)
    res->p[k] = u->p[k] * m;
}

/// @brief res = u - v
void vec3_sub(struct vec3d *res, const struct vec3d *u, const struct vec3d *v) {
  for (int k = 0; k < 3; ++k)
    res->p[k] = u->p[k] - v->p[k];
}

/// @brief res = u + v
void vec3_add(struct vec3d *res, const struct vec3d *u, const struct vec3d *v) {
  for (int k = 0; k < 3; ++k)
    res->p[k] = u->p[k] + v->p[k];
}
...
void mat3_mul(struct mat3d *res, const struct mat3d *_a,
              const struct mat3d *_b) {
...
void mat3_vmul(struct vec3d *res, const struct mat3d *a,
               const struct vec3d *v) {
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
      res->p[i] = a->m[i][j] * v->p[j];
}
void vec3_outer(struct mat3d *res, const struct vec3d *u,
                const struct vec3d *v) {
  // outer product
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
