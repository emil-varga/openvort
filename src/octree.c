...
struct octree *octree_init(int Ninit, int depth, int quadtree) {
  ...
  const int children_N =
      tree->quadtree > 0 ? OCTREE_2D_CHILDREN_N : OCTREE_CHILDREN_N;
...
struct octree *octree_add_node(struct octree *tree, int node_id) {
  ...
  tree->node_ids =
      (int *)realloc(tree->node_ids, 2 * tree->N_total * sizeof(int));
  if (tree->node_ids) {
    tree->N_total *= 2;
    tree->node_ids[tree->N++] = node_id;
    return tree;
...
int octree_inner_sort(struct octree *tree, const struct tangle_state *tangle) {
  if (!tree ||
      tree->N <= 1) // there is only one node, we do not need to split further
    return 0;
  ...
  for (int k = 0; k < tree->N; ++k) {
    for (octree_child_idx child = 0; child < children_N; child++) {
      if (in_box(&tree->children[child]->box,
                 &tangle->vnodes[tree->node_ids[k]])) {
  ...
  // update the centers of mass, total circulation vectors and circulation
  // tensors
  octree_update_means(tree, tangle);
  return new_children;
}

struct domain_box make_child_box(const struct domain_box *parent_box,
                                 octree_child_idx child_idx, int quadtree);
void octree_make_child_boxes(struct octree *tree) {
  ...
  for (octree_child_idx child = 0; child < children_N; ++child) {
    struct domain_box child_box =
        make_child_box(&tree->box, child, tree->quadtree);
    tree->children[child] =
        octree_init(Ninit, 1, tree->quadtree); // do not create any depth here
    tree->children[child]->box = child_box;
    tree->children[child]->tangle = tree->tangle;
  }
}
// Helper for creating a child box. Gets the base dimensions from parent_box and
// returns the box corresponding to the child_idx.
struct domain_box make_child_box(const struct domain_box *parent_box,
                                 octree_child_idx child_idx, int quadtree) {
...
void octree_update_means(struct octree *tree,
  ...
  for (int k = 0; k < tree->N; k++) {
    vec3_add(&tree->centre_of_mass, &tree->centre_of_mass,
             &tangle->vnodes[tree->node_ids[k]]);
  }
  ...
  for (int k = 0; k < tree->N; k++) {
    ...
    struct segment seg =
        seg_pwrap(&tangle->vnodes[idx], &tangle->vnodes[forward], &tangle->box);
  ...
  const int children_N =
      tree->quadtree ? OCTREE_2D_CHILDREN_N : OCTREE_CHILDREN_N;
  for (octree_child_idx child = 0; child < children_N; child++)
    octree_update_means(tree->children[child], tangle);
}

// velocity calculation at point r
void octree_get_vs(const struct octree *tree, const struct vec3d *r,
                   double resolution, struct vec3d *res, int skip) {
  if (!tree || tree->N == 0) {
    // empty leaf, exit
    *res = vec3(0, 0, 0);
    return;
  ...
  if (tree->N == 1 || Lmin < global_BH_grain) {
    ...
    *res = calculate_vs_shift(tree->tangle, *r, skip, NULL, tree->node_ids,
                              tree->N);
    return;
  }
