...
void add_line_KW(struct tangle_state *tangle, double x, double y, int direction,
  ...
  struct vec3d s = vec3(x + r_KW * cos(k_KW * zstart),
                        y + r_KW * sin(k_KW * zstart), zstart);
  struct vec3d sp =
      vec3(-k_KW * r_KW * sin(k_KW * zstart), k_KW * r_KW * cos(k_KW * zstart),
           direction); // tangent
  struct vec3d spp =
      vec3(-k_KW * k_KW * r_KW * cos(k_KW * zstart),
           -k_KW * k_KW * r_KW * sin(k_KW * zstart), 0); // normal

  const double s_mag = vec3_d(&sp); // the magnitude of the derivative doesn't
                                    // change, only its direction
  ...
  if (tangle->box.wall[start_wall] == WALL_MIRROR) {
    ...
    tangle->vnodes[new_pt].p[2] +=
        direction * 1e-6; // so that it's not right on the boundary
...
void write_vector(FILE *stream, struct vec3d *v) {
  fprintf(stream, "%.15g\t%.15g\t%.15g", v->p[0], v->p[1], v->p[2]);
}

void save_point(FILE *stream, int vort_idx, const struct tangle_state *tangle,
                int i) {
  ...
  fprintf(stream, "\t%d\t%d\t%d\t%d\t%d\t%.15g", i,
          tangle->connections[i].reverse, tangle->connections[i].forward,
          tangle->status[i].status, tangle->status[i].pin_wall, tangle->dxi[i]);
  fprintf(stream, "\n");
}
...
int load_point(FILE *stream, struct tangle_state *tangle, int idx) {
  ...
  int ret = fscanf(
      stream, "%d\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg\t%lg",
      &vidx, pos->p, pos->p + 1, pos->p + 2, vel->p, vel->p + 1, vel->p + 2,
      tangent->p, tangent->p + 1, tangent->p + 2, normal->p, normal->p + 1,
      normal->p + 2);
  if (ret == EOF)
    return -1;
...
// point is corrupted
  ...
  printf("Corrupted links in point %d (%d, %d)\n", k,
         tangle->connections[k].forward, tangle->connections[k].reverse);
#endif
  return -1;
}
...
double wall_dist(const struct tangle_state *tangle, int k,
                 boundary_faces wall) {
  /*
  ...
  case Z_L:
    return tangle->vnodes[k].p[idx[wall]] -
           tangle->box.bottom_left_back.p[idx[wall]];

  case X_H:
  case Y_H:
  case Z_H:
    return tangle->box.top_right_front.p[idx[wall]] -
           tangle->vnodes[k].p[idx[wall]];
