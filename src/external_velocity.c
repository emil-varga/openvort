...
int get_v_param_scalar(const struct v_conf_t *vconf, const char *name,
                       double *res) {
...
int get_v_param_vector(const struct v_conf_t *vconf, const char *name,
                       struct vec3d *res) {
  for (int k = 0; k < vconf->n_params; ++k) {
    if (strcmp(name, vconf->v_params[k].name) == 0) {
...
const struct v_conf_t v_conf_noflow = {.name = "no flow",
                                       .fun = get_v_noflow,
                                       .n_params = 0,
...
struct v_conf_t v_confs[] = {
    ...
    {.name = "spherical",
     .fun = get_v_spherical,
     .n_params = 2,
     .v_params = {{"strength", scalar_param, {.scalar = 0}},
                  {"cutoff", scalar_param, {.scalar = 0}}}},
    {.name = "simple",
     .fun = get_v_simple,
     .n_params = 1,
     .v_params = {{"external_v", vector_param, {.scalar = 0}}}},
    ...
    {.name = "coscos",
     .fun = get_v_coscos,
     .n_params = 3,
     .v_params = {{"k", scalar_param, {.scalar = 0}},
    ...
    {.name = "simple-shear",
     ...
     .n_params = 2,
     .v_params = {{"rate", scalar_param, {.scalar = 0}},
                  {"v0", scalar_param, {.scalar = 0}}}},
    {.name = "rotosc",
     .fun = get_v_rotosc,
     .n_params = 2,
     .v_params = {{"freq", scalar_param, {.scalar = 0}},
                  {"amp", scalar_param, {.scalar = 0}}}},
    {.name = "DG",
     .fun = get_v_DG,
     .n_params = 2,
     .v_params = {{"rot", scalar_param, {.scalar = 0}},
    ...
    {.name = "constant acceleration",
     ...
     .n_params = 1,
     .v_params = {{"accel", scalar_param, {.scalar = 0}}}},
    {.name = "parabolic",
     .fun = get_v_parabolic,
     .n_params = 2,
     .v_params = {{"H", scalar_param, {.scalar = 0}},
                  {"amp", scalar_param, {.scalar = 0}}}},
    {.name = "",
     .fun = NULL,
     .n_params = 0,
...
int get_v_noflow(const struct vec3d *where __attribute__((unused)),
                 double t __attribute__((unused)), struct vec3d *res,
                 struct v_conf_t *v_conf __attribute__((unused))) {
  *res = vec3(0, 0, 0); // no flow

  return 0;
}

int get_v_simple(const struct vec3d *where __attribute__((unused)),
                 double t __attribute__((unused)), struct vec3d *res,
                 struct v_conf_t *vconf) {
  return get_v_param_vector(vconf, "external_v", res);
}

/*
 * Returns a spherical sink (negative strength) or source (positive strength)
 * velocity field. The singularity is at origin and the magnitude is strength/(4
 * pi r^2) where r is the distance from the origin to he point of interest
 * (*where).
 */

int get_v_spherical(const struct vec3d *where, double t __attribute__((unused)),
                    struct vec3d *res, struct v_conf_t *vconf) {
  double strength;
  double cutoff;
  int err;
  ...
  if (r < 1e-8 || attn > 100) // hard cutoff to prevent 1/0 or underflow SIGFPE,
...
int get_v_spherical_and_simple(const struct vec3d *where,
...
int get_v_cylindrical(const struct vec3d *where,
                      double t __attribute__((unused)), struct vec3d *res,
                      struct v_conf_t *vconf) {
...
int get_v_oscillating(const struct vec3d *where, double t, struct vec3d *res,
                      struct v_conf_t *vconf) {
  double freq;
  double strength;
  struct vec3d k;
...
int get_v_oscillating_slit(const struct vec3d *where, double t,
                           struct vec3d *res, struct v_conf_t *vconf) {
  double freq;
  double strength;
  double cutoff;
...
int get_v_coscos(const struct vec3d *where, double t __attribute__((unused)),
                 struct vec3d *res, struct v_conf_t *vconf) {
  double k, v0, l;
...
int get_v_coscos_divfree(const struct vec3d *where,
                         double t __attribute__((unused)), struct vec3d *res,
                         struct v_conf_t *vconf) {
  double k, v0, l;
...
int get_v_cos_divfree(const struct vec3d *where,
                      double t __attribute__((unused)), struct vec3d *res,
                      struct v_conf_t *vconf) {
  double k, v0, l;
...
int get_v_simple_shear(const struct vec3d *where,
                       double t __attribute__((unused)), struct vec3d *res,
                       struct v_conf_t *vconf) {
  double rate;
  double v0;
...
int get_v_rotosc(const struct vec3d *where, double t, struct vec3d *res,
                 struct v_conf_t *vconf) {
  double freq;
  double amp;
...
*/
int get_v_DG(const struct vec3d *where, double t __attribute__((unused)),
             struct vec3d *res, struct v_conf_t *vconf) {
  double rot;
  double vz;
...
int get_v_rotosc_boundary(const struct vec3d *where, double t,
                          struct vec3d *res, struct v_conf_t *vconf) {
  double freq;
  double amp;
  double delta;
...
int get_v_constant_acceleration(const struct vec3d *where
                                __attribute__((unused)),
                                double t, struct vec3d *res,
                                struct v_conf_t *vconf) {
  double acc;
  int err;
...
int get_v_parabolic(const struct vec3d *where, double t, struct vec3d *res,
                    struct v_conf_t *vconf) {
  double vx;
  double amp;
  double H;
