...
struct vec3d shifted(const struct image_tangle *shift,
                     const struct tangle_state *tangle, const struct vec3d *r) {
  /*
  ...
  for (int k = 0; k < 3; ++k)
    Ls[k] =
        tangle->box.top_right_front.p[k] - tangle->box.bottom_left_back.p[k];
...
void create_tangle(struct tangle_state *tangle, size_t n) {
  ...
  struct domain_box box = {.bottom_left_back = {{0, 0, 0}},
                           .top_right_front = {{1, 1, 1}},
                           .wall = {WALL_OPEN, WALL_OPEN, WALL_OPEN, WALL_OPEN,
                                    WALL_OPEN, WALL_OPEN}};
...
void expand_tangle(struct tangle_state *tangle, size_t n) {
  ...
  tangle->vnodes =
      (struct vec3d *)realloc(tangle->vnodes, sizeof(struct vec3d) * n);
  tangle->vnodes_new =
      (struct vec3d *)realloc(tangle->vnodes_new, sizeof(struct vec3d) * n);
  tangle->vels =
      (struct vec3d *)realloc(tangle->vels, sizeof(struct vec3d) * n);
  tangle->vs = (struct vec3d *)realloc(tangle->vs, sizeof(struct vec3d) * n);
  tangle->tangents =
      (struct vec3d *)realloc(tangle->tangents, sizeof(struct vec3d) * n);
  tangle->normals =
      (struct vec3d *)realloc(tangle->normals, sizeof(struct vec3d) * n);
...
struct vec3d step_node(const struct tangle_state *tangle, int i, int where) {
  ...
  if (tangle->status[i].status == FREE) {
    ...
    // if we are here it means we have ran into a wall and we need to flip the
    // node
...
/// @brief Similar to step_node, but integrates the distance along the vortex
/// using arc length deltas from tangle state
...
double arc_length(const struct tangle_state *tangle, int k, int dist) {
  ...
  if (tangle->status[k].status == FREE) {
    if (dist > 0)
      return tangle->dxi[k] +
             arc_length(tangle, tangle->connections[k].forward, dist - 1);
    else if (dist < 0)
      return arc_length(tangle, tangle->connections[k].reverse, +1) +
             arc_length(tangle, tangle->connections[k].reverse, dist + 1);
  } else if (tangle->status[k].status == PINNED ||
...
static void update_pinned_node_tangent_normal(struct tangle_state *tangle,
                                              size_t k) {
  ...
  double c1 =
      d2 * d3 * d4 /
      (d1 * (d1 * d1 * d1 - d1 * d1 * d2 - d1 * d1 * d3 - d1 * d1 * d4 +
             d1 * d2 * d3 + d1 * d2 * d4 + d1 * d3 * d4 - d2 * d3 * d4));
  double c2 =
      -d1 * d3 * d4 /
      (d2 * (d1 * d2 * d2 - d1 * d2 * d3 - d1 * d2 * d4 + d1 * d3 * d4 -
             d2 * d2 * d2 + d2 * d2 * d3 + d2 * d2 * d4 - d2 * d3 * d4));
  double c3 =
      d1 * d2 * d4 /
      (d3 * (d1 * d2 * d3 - d1 * d2 * d4 - d1 * d3 * d3 + d1 * d3 * d4 -
             d2 * d3 * d3 + d2 * d3 * d4 + d3 * d3 * d3 - d3 * d3 * d4));
  double c4 =
      -d1 * d2 * d3 /
      (d4 * (d1 * d2 * d3 - d1 * d2 * d4 - d1 * d3 * d4 + d1 * d4 * d4 -
             d2 * d3 * d4 + d2 * d4 * d4 + d3 * d4 * d4 - d4 * d4 * d4));
  ...
  n = vec3(0, 0, 0);
  for (int j = 0; j < 4; ++j) {
    vec3_mul(&tmp, ds[j], cs[j]);
    vec3_add(&t, &t, &tmp);

    vec3_mul(&tmp, ds[j], cns[j]);
    vec3_add(&n, &n, &tmp);
  }
...
static void update_pinned_neighbor_tangent_normal(struct tangle_state *tangle,
  ...
  double ct_ds1 = 2 * dm1 * dm1 * (-2 * d1 + dm1) /
                  (d1 * (d1 * d1 * d1 - 3 * d1 * d1 * dm1 + 3 * d1 * dm1 * dm1 -
                         dm1 * dm1 * dm1));
  double ct_dsm1 = 2 * d1 * d1 * (-d1 + 2 * dm1) /
                   (dm1 * (d1 * d1 * d1 - 3 * d1 * d1 * dm1 +
                           3 * d1 * dm1 * dm1 - dm1 * dm1 * dm1));

  double cn_ds1 = 2 * dm1 * (8 * d1 * d1 - d1 * dm1 - dm1 * dm1) /
                  (d1 * d1 *
                   (d1 * d1 * d1 - 3 * d1 * d1 * dm1 + 3 * d1 * dm1 * dm1 -
                    dm1 * dm1 * dm1));
  double cn_dsm1 = 2 * d1 * (d1 * d1 + d1 * dm1 - 8 * dm1 * dm1) /
                   (dm1 * dm1 *
                    (d1 * d1 * d1 - 3 * d1 * d1 * dm1 + 3 * d1 * dm1 * dm1 -
                     dm1 * dm1 * dm1));

  double cn_t1 =
      2 * dm1 * (2 * d1 + dm1) / (d1 * (d1 * d1 - 2 * d1 * dm1 + dm1 * dm1));
  double cn_tm1 =
      2 * d1 * (d1 + 2 * dm1) / (dm1 * (d1 * d1 - 2 * d1 * dm1 + dm1 * dm1));
...
static void update_tangent_normal_internal(struct tangle_state *tangle, size_t k, char *visited) {
  ...
  double s_1_cf[] = {
      -d1 * dm1 * dm2 /
          (d2 *
           (d1 * d2 * d2 - d1 * d2 * dm1 - d1 * d2 * dm2 + d1 * dm1 * dm2 -
            d2 * d2 * d2 + d2 * d2 * dm1 + d2 * d2 * dm2 - d2 * dm1 * dm2)),
      d2 * dm1 * dm2 /
          (d1 *
           (d1 * d1 * d1 - d1 * d1 * d2 - d1 * d1 * dm1 - d1 * d1 * dm2 +
            d1 * d2 * dm1 + d1 * d2 * dm2 + d1 * dm1 * dm2 - d2 * dm1 * dm2)),
      d1 * d2 * dm2 /
          (dm1 * (d1 * d2 * dm1 - d1 * d2 * dm2 - d1 * dm1 * dm1 +
                  d1 * dm1 * dm2 - d2 * dm1 * dm1 + d2 * dm1 * dm2 +
                  dm1 * dm1 * dm1 - dm1 * dm1 * dm2)),
      -d1 * d2 * dm1 /
          (dm2 * (d1 * d2 * dm1 - d1 * d2 * dm2 - d1 * dm1 * dm2 +
                  d1 * dm2 * dm2 - d2 * dm1 * dm2 + d2 * dm2 * dm2 +
                  dm1 * dm2 * dm2 - dm2 * dm2 * dm2))};

  // second derivative
  // four point coefficients O(d^3)
  double s_2_cf[] = {
      2 * (d1 * dm1 + d1 * dm2 + dm1 * dm2) /
          (d2 *
           (d1 * d2 * d2 - d1 * d2 * dm1 - d1 * d2 * dm2 + d1 * dm1 * dm2 -
            d2 * d2 * d2 + d2 * d2 * dm1 + d2 * d2 * dm2 - d2 * dm1 * dm2)),
      2 * (-d2 * dm1 - d2 * dm2 - dm1 * dm2) /
          (d1 *
           (d1 * d1 * d1 - d1 * d1 * d2 - d1 * d1 * dm1 - d1 * d1 * dm2 +
            d1 * d2 * dm1 + d1 * d2 * dm2 + d1 * dm1 * dm2 - d2 * dm1 * dm2)),
      2 * (-d1 * d2 - d1 * dm2 - d2 * dm2) /
          (dm1 * (d1 * d2 * dm1 - d1 * d2 * dm2 - d1 * dm1 * dm1 +
                  d1 * dm1 * dm2 - d2 * dm1 * dm1 + d2 * dm1 * dm2 +
                  dm1 * dm1 * dm1 - dm1 * dm1 * dm2)),
      2 * (d1 * d2 + d1 * dm1 + d2 * dm1) /
          (dm2 * (d1 * d2 * dm1 - d1 * d2 * dm2 - d1 * dm1 * dm2 +
                  d1 * dm2 * dm2 - d2 * dm1 * dm2 + d2 * dm2 * dm2 +
                  dm1 * dm2 * dm2 - dm2 * dm2 * dm2))};
...
/*
 * Calculates the velocity field of a straight vortex segment given by seg at
 * position r
 */
 ...
 */
static inline struct vec3d segment_field(const struct tangle_state *tangle,
                                         size_t i, struct vec3d r) {
...
static inline struct vec3d lia_velocity(const struct tangle_state *tangle,
                                        int i) {
  ...
  double f = KAPPA *
             log(2 * sqrt(l_next * l_prev) / sqrt(sqrt(M_E)) / VORTEX_WIDTH) /
             4 / M_PI;
...
struct vec3d calculate_vs_shift(const struct tangle_state *tangle,
                                struct vec3d r, int skip,
...
void update_velocity(struct tangle_state *tangle, int k, double t,
  ...
  if (global_hyperfriction) {
    // hyperfriction damps segments of vortices with very high curvature, idea
    // is similar to hyperviscosity dissipation similar to mutual friction is
    // applied with high alpha (~0.5 -- 1) but only to parts where curvature is
    // higher than settable threshold and only through the (curvature-sensitive)
    // locally induced velocity
    struct vec3d tmp;
...
static inline int out_of_box(const struct tangle_state *tangle,
                             const struct vec3d *where) {
  const double x = where->p[0];
  const double y = where->p[1];
  const double z = where->p[2];
  const double bounds[] = {
      tangle->box.bottom_left_back.p[0], tangle->box.top_right_front.p[0],
...
void remesh(struct tangle_state *tangle, double min_dist, double max_dist) {
  ...
  for (int k = 0; k < tangle->N; ++k) {
    ...
    if (next >= 0 && prev >= 0 &&
        !(tangle->connections[prev].reverse < 0 &&
          tangle->connections[next].forward < 0) &&
...
void eliminate_small_loops(struct tangle_state *tangle, int loop_length) {
  ...
  if (global_eliminate_zaxis_loops)
    eliminate_loops_near_zaxis(tangle, global_eliminate_loops_zaxis_cutoff,
                               1); // inside removal
  ...
  for (int k = 0; k < tangle->N; ++k) {
    ...
    while (next != k) {
      if ((tangle->status[here].status == PINNED ||
           tangle->status[here].status == PINNED_SLIP) &&
          next < 0) {
...
int add_point(struct tangle_state *tangle, int p) {
  ...
  if (nd < global_max_curvature_scale / global_dl_max && nd > 1e-8) {
    // if the curvature is too high (can happen near walls due to numerical
    // instability) this extrapolation places the point too far and breaks the
    // curvature calculation in the subsequent steps n will be identically 0 for
    // a straight vortex for both of these cases it's better to just average s0
    // and s1
...
int curvature_smoothing(struct tangle_state *tangle, double max_spp,
                        double damping) {
  update_tangents_normals(tangle);
