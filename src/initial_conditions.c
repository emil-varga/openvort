...
void random_straight_lines(struct tangle_state *tangle, int npairs,
                           int points_per_line) {
  // for the slab geometry, populate the computational box with straight
  // vortices
  assert((tangle->box.wall[Z_L] == WALL_MIRROR &&
...
void random_polarized_lines(struct tangle_state *tangle, int nvort,
                            int direction, int points_per_line) {
  assert((tangle->box.wall[Z_L] == WALL_MIRROR &&
          tangle->box.wall[Z_H] == WALL_MIRROR) ||
         (tangle->box.wall[Z_L] == WALL_PERIODIC &&
          tangle->box.wall[Z_H] == WALL_PERIODIC));
...
void triangular_lattice(struct tangle_state *tangle, int shells, int direction,
                        int points_per_line, float lattice_constant) {
  ...
  for (int i = -shells; i < shells + 1; ++i) {
    for (int j = -shells; j < shells + 1; ++j) {
      ...
      double x = i - 0.5 * j;
      double y = dy * j;
      add_line(tangle, x * lattice_constant, y * lattice_constant, direction,
               points_per_line);
    }
  }
}
...
void dipole_straight_lines(struct tangle_state *tangle, int points_per_line,
                           int direction, double separation) {
  assert((tangle->box.wall[Z_L] == WALL_MIRROR &&
          tangle->box.wall[Z_H] == WALL_MIRROR) ||
         (tangle->box.wall[Z_L] == WALL_PERIODIC &&
          tangle->box.wall[Z_H] == WALL_PERIODIC));
...
void single_straight_line(struct tangle_state *tangle, int points_per_line,
                          int direction, int k_KW, double r_KW) {
  assert((tangle->box.wall[Z_L] == WALL_MIRROR &&
          tangle->box.wall[Z_H] == WALL_MIRROR) ||
         (tangle->box.wall[Z_L] == WALL_PERIODIC &&
          tangle->box.wall[Z_H] == WALL_PERIODIC));
