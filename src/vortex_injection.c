...
void inject_vortices(struct tangle_state *tangle, double t) {
  ...
  if (global_line_injection)
    inject_line_pairs(tangle, t, global_line_injection_frequency,
                      global_line_injection_n, global_line_injection_polarized);
}
...
void inject_loop(struct tangle_state *tangle, double t, double frequency) {
  ...
  struct vec3d dir =
      vec3(2 * (drand48() - 0.5), 2 * (drand48() - 0.5), -drand48());
  vec3_normalize(&dir);
  struct vec3d cent;
...
void inject_line_pairs(struct tangle_state *tangle, double t, double frequency,
  ...
  /*
   * Injects a pair of straight vortex lines of opposite circulations oriented
   * along z-axis. Only makes sense for the 2-wall and periodic boundaries.
   */
